...
`config.fish` sets environment variables, PATH, rbenv init, and Claude Code settings (`DISABLE_NON_ESSENTIAL_MODEL_CALLS=1`). Docker aliases are in a separate `docker.fish` file. Plugins managed via Fisher: `z` (directory jumping) and `tide@v5` (prompt).
...
30+ aliases defined. Uses `nvimdiff` as merge/diff tool. GHQ root at `~/.ghq`. Configured for `git log` with graph formatting.
